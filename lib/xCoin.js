...
function sellCoin(currency, units){
  var rgParams = {
    currency: currency,
    units: String(units)
	};

	xcoinAPI.xcoinApiCall('/trade/market_sell', rgParams);
}

function buyCoin(currency, units){
  var rgParams = {
    currency: currency,
    units: String(units)
	};

	xcoinAPI.xcoinApiCall('/trade/market_buy', rgParams);
}

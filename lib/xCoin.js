...
function init(){
	xcoinAPI = new XCoinAPI('api', 'apisecret');
	console.log('bithumb private api init');
}
   
function sellCoin(currency, units, cb){
  var rgParams = {
    currency: currency,
    units: String(units)
	};

	xcoinAPI.xcoinApiCall('/trade/market_sell', rgParams, cb);
}

function buyCoin(currency, units, cb){
  var rgParams = {
    currency: currency,
    units: String(units)
	};

	xcoinAPI.xcoinApiCall('/trade/market_buy', rgParams, cb);
}

function getMyBalance(cb){
	xcoinAPI = new XCoinAPI('2c03cd7ac26495d634569ed852c4353f', '59f07a5e26b83f6e1d495774315e2a8b');

  var rgParams = {
    currency: 'ALL',
	};

	xcoinAPI.xcoinApiCall('/info/balance', rgParams, cb);
}
...
XCoinAPI.prototype.xcoinApiCall = function(endPoint, params, cb) {
	var rgParams = {
		'endPoint' : endPoint
	};
	...
	var rgResult = this.request(api_host, 'POST', rgParams, httpHeaders, cb);
}

XCoinAPI.prototype.request = function(strHost, strMethod, rgParams, httpHeaders, cb) {
	var rgHeaders = {};
	if(httpHeaders) {
	...
	function(error, response, rgResult) {
		...
		try {
			...
			rgResultDecode.status = 'ffff';
			rgResultDecode.message = 'invaild rgResult';
		}
